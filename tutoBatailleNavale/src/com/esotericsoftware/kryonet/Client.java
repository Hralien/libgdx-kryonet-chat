		...
		while (!shutdown) {
			try {
				...
				if (TRACE) {
					if (isConnected)
						trace("kryonet",
								"Unable to update connection: " + this, ex);
					else
	...
	private void broadcast(int udpPort, DatagramSocket socket)
		...
		for (NetworkInterface iface : Collections.list(NetworkInterface
			...
			for (InetAddress address : Collections.list(iface
				...
				if (InetAddress.getByAddress(ip) instanceof Inet4Address) {
					ip[3] = -1; // 255.255.255.0
//					// System.err.println(ip.length +"\n");
//					for (int i = 0; i < ip.length; i++) {
//						System.out.print(ip[i] + ".");
//					}
//					System.err.println(InetAddress.getByAddress(ip));
//					System.out.println("");
					try {
			...
			if (DEBUG)
				debug("kryonet", "Broadcasted host discovery on port: "
						+ udpPort);
		}
	}
